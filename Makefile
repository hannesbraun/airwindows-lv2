.POSIX:

SOURCES=src/manifest.ttl.in $(src/*/*) meson.build

all: airwindows-lv2-amd64-macos.tar.gz airwindows-lv2-aarch64-macos.tar.gz airwindows-lv2-amd64-windows.zip airwindows-lv2-i686-windows.zip

airwindows-lv2-amd64-macos.tar.gz: $(SOURCES)
	rm -rf build-amd64-macos
	meson setup build-amd64-macos
	meson compile -C build-amd64-macos
	meson install --destdir . -C build-amd64-macos
	test -d build-amd64-macos/usr/local/lib/lv2/Airwindows.lv2
	mkdir airwindows-lv2-amd64-macos
	cp -r build-amd64-macos/usr/local/lib/lv2/Airwindows.lv2 airwindows-lv2-amd64-macos
	rm -rf build-amd64-macos
	cp LICENSE airwindows-lv2-amd64-macos
	tar -cvzf airwindows-lv2-amd64-macos.tar.gz airwindows-lv2-amd64-macos
	rm -r airwindows-lv2-amd64-macos

airwindows-lv2-aarch64-macos.tar.gz: $(SOURCES) cross/macos-aarch64.txt
	rm -rf build-aarch64-macos
	meson setup --cross-file cross/macos-aarch64.txt build-aarch64-macos
	meson compile -C build-aarch64-macos
	meson install --destdir . -C build-aarch64-macos
	test -d build-aarch64-macos/usr/local/lib/lv2/Airwindows.lv2
	mkdir airwindows-lv2-aarch64-macos
	cp -r build-aarch64-macos/usr/local/lib/lv2/Airwindows.lv2 airwindows-lv2-aarch64-macos
	rm -rf build-aarch64-macos
	cp LICENSE airwindows-lv2-aarch64-macos
	tar -cvzf airwindows-lv2-aarch64-macos.tar.gz airwindows-lv2-aarch64-macos
	rm -r airwindows-lv2-aarch64-macos

airwindows-lv2-amd64-windows.zip: $(SOURCES) cross/windows-amd64.txt
	rm -rf build-amd64-windows
	meson setup --cross-file cross/windows-amd64.txt build-amd64-windows
	meson compile -C build-amd64-windows
	meson install --destdir . -C build-amd64-windows
	test -d build-amd64-windows/usr/local/lib/lv2/Airwindows.lv2
	mkdir airwindows-lv2-amd64-windows
	cp -r build-amd64-windows/usr/local/lib/lv2/Airwindows.lv2 airwindows-lv2-amd64-windows
	rm -rf build-amd64-windows
	cp LICENSE airwindows-lv2-amd64-windows
	zip -9 -v -r airwindows-lv2-amd64-windows.zip airwindows-lv2-amd64-windows
	rm -r airwindows-lv2-amd64-windows

airwindows-lv2-i686-windows.zip: $(SOURCES) cross/windows-i686.txt
	rm -rf build-i686-windows
	meson setup --cross-file cross/windows-i686.txt build-i686-windows
	meson compile -C build-i686-windows
	meson install --destdir . -C build-i686-windows
	test -d build-i686-windows/usr/local/lib/lv2/Airwindows.lv2
	mkdir airwindows-lv2-i686-windows
	cp -r build-i686-windows/usr/local/lib/lv2/Airwindows.lv2 airwindows-lv2-i686-windows
	rm -rf build-i686-windows
	cp LICENSE airwindows-lv2-i686-windows
	zip -9 -v -r airwindows-lv2-i686-windows.zip airwindows-lv2-i686-windows
	rm -r airwindows-lv2-i686-windows

.PHONY: all
