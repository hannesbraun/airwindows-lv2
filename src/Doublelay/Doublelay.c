...
static void activate(LV2_Handle instance)
{
	...
	for (int count = 0; count < 48009; count++) {
		doublelay->dL[count] = 0.0;
		doublelay->dR[count] = 0.0;
	}
	doublelay->dcount = 0;
	for (int count = 0; count < 5009; count++) {
		doublelay->pL[count] = 0.0;
		doublelay->pR[count] = 0.0;
	}
	for (int count = 0; count < 8; count++) {
		doublelay->tempL[count] = 0.0;
		doublelay->positionL[count] = 0.0;
		doublelay->lastpositionL[count] = 0.0;
		doublelay->trackingL[count] = 0.0;
	}
	for (int count = 0; count < 8; count++) {
		doublelay->tempR[count] = 0.0;
		doublelay->positionR[count] = 0.0;
		doublelay->lastpositionR[count] = 0.0;
		doublelay->trackingR[count] = 0.0;
	}
	doublelay->gcountL = 0;
	...
	for (int count = 0; count < 6; count++) {
		doublelay->lastRefL[count] = 0.0;
		doublelay->lastRefR[count] = 0.0;
	}
	doublelay->cycle = 0;

	doublelay->fpdL = 1.0;
	while (doublelay->fpdL < 16386) doublelay->fpdL = rand() * UINT32_MAX;
	doublelay->fpdR = 1.0;
	while (doublelay->fpdR < 16386) doublelay->fpdR = rand() * UINT32_MAX;
}
...
static void run(LV2_Handle instance, uint32_t sampleFrames)
{
	...
	while (sampleFrames-- > 0) {
		...
		if (doublelay->cycle == cycleEnd) { // hit the end point and we do a doubler sample
			...
			if (doublelay->flip) {
				doublelay->airEvenL += doublelay->airFactorL;
				doublelay->airOddL -= doublelay->airFactorL;
				doublelay->airFactorL = doublelay->airEvenL;
			} else {
				doublelay->airOddL += doublelay->airFactorL;
				doublelay->airEvenL -= doublelay->airFactorL;
				doublelay->airFactorL = doublelay->airOddL;
			}
			...
			if (doublelay->flip) {
				doublelay->airEvenR += doublelay->airFactorR;
				doublelay->airOddR -= doublelay->airFactorR;
				doublelay->airFactorR = doublelay->airEvenR;
			} else {
				doublelay->airOddR += doublelay->airFactorR;
				doublelay->airEvenR -= doublelay->airFactorR;
				doublelay->airFactorR = doublelay->airOddR;
			}
			...
			for (count = 0; count < 8; count++) {
				doublelay->positionL[count] += speedL;
				doublelay->positionR[count] += speedR;
			}
			...
			if (doublelay->trackingL[doublelay->activeL] == 0.0) {
				...
				if ((gplusL > doublelay->positionL[doublelay->activeL]) && (lastplusL < doublelay->lastpositionL[doublelay->activeL])) {
					doublelay->trackingL[doublelay->activeL] = 1.0;
				}
				if ((posplusL > doublelay->gcountL) && (lastposplusL < doublelay->lastcountL)) {
					doublelay->trackingL[doublelay->activeL] = 1.0;
				}
			...
			if (doublelay->trackingR[doublelay->activeR] == 0.0) {
				...
				if ((gplusR > doublelay->positionR[doublelay->activeR]) && (lastplusR < doublelay->lastpositionR[doublelay->activeR])) {
					doublelay->trackingR[doublelay->activeR] = 1.0;
				}
				if ((posplusR > doublelay->gcountR) && (lastposplusR < doublelay->lastcountR)) {
					doublelay->trackingR[doublelay->activeR] = 1.0;
				}
				// fire splice based on whether somebody moved past somebody else just now
			}
			...
			for (count = 0; count < 8; count++) {
				if (doublelay->positionL[count] > width) {
					doublelay->positionL[count] -= width;
				}
				if (doublelay->positionR[count] > width) {
					doublelay->positionR[count] -= width;
				}
				doublelay->lastpositionL[count] = doublelay->positionL[count];
				doublelay->lastpositionR[count] = doublelay->positionR[count];
			}
			if (doublelay->gcountL < 0 || doublelay->gcountL > width) {
				doublelay->gcountL -= width;
			}
			doublelay->lastcountL = bcountL = doublelay->gcountL;
			if (doublelay->gcountR < 0 || doublelay->gcountR > width) {
				doublelay->gcountR -= width;
			}
			...
			if (doublelay->trackingL[doublelay->activeL] > 0.0) {
				...
				for (count = 0; count < 8; count++) {
					if ((doublelay->trackingL[count] < doublelay->bestyetL) && (count != doublelay->activeL)) {
						doublelay->bestspliceL = count;
						doublelay->bestyetL = doublelay->trackingL[count];
					}
				}
			...
			if (doublelay->trackingR[doublelay->activeR] > 0.0) {
				...
				for (count = 0; count < 8; count++) {
					...
					if ((depth > 0) && (count != doublelay->activeR)) {
						doublelay->trackingR[count] -= (depth / adjust);
						doublelay->bestspliceR = count;
					}
					// take down the splicings but skip the current one
				}
				doublelay->bestyetR = 1.0;
				for (count = 0; count < 8; count++) {
					if ((doublelay->trackingR[count] < doublelay->bestyetR) && (count != doublelay->activeR)) {
						doublelay->bestspliceR = count;
						doublelay->bestyetR = doublelay->trackingR[count];
					}
				}
		...
		doublelay->fpdL ^= doublelay->fpdL << 13;
		doublelay->fpdL ^= doublelay->fpdL >> 17;
		doublelay->fpdL ^= doublelay->fpdL << 5;
		inputSampleL += (((double) doublelay->fpdL - (uint32_t) 0x7fffffff) * 5.5e-36l * pow(2, expon + 62));
		frexpf((float) inputSampleR, &expon);
		doublelay->fpdR ^= doublelay->fpdR << 13;
		doublelay->fpdR ^= doublelay->fpdR >> 17;
		doublelay->fpdR ^= doublelay->fpdR << 5;
