...
static void activate(LV2_Handle instance)
{
	...
	for(int count = 0; count < 34; count++) {luxor->bR[count] = 0;luxor->bL[count] = 0;}
	luxor->lastSampleR = 0.0;luxor->lastSampleL = 0.0;
	luxor->fpdL = 1.0; while (luxor->fpdL < 16386) luxor->fpdL = rand()*UINT32_MAX;
	luxor->fpdR = 1.0; while (luxor->fpdR < 16386) luxor->fpdR = rand()*UINT32_MAX;
}
...
static void run(LV2_Handle instance, uint32_t sampleFrames)
{
	...
	while (sampleFrames-- > 0) {
		...
		if (sqdrive > 0.0){
			luxor->bL[33] = luxor->bL[32]; luxor->bL[32] = luxor->bL[31];
			luxor->bL[31] = luxor->bL[30]; luxor->bL[30] = luxor->bL[29]; luxor->bL[29] = luxor->bL[28]; luxor->bL[28] = luxor->bL[27]; luxor->bL[27] = luxor->bL[26]; luxor->bL[26] = luxor->bL[25]; luxor->bL[25] = luxor->bL[24]; luxor->bL[24] = luxor->bL[23];
			luxor->bL[23] = luxor->bL[22]; luxor->bL[22] = luxor->bL[21]; luxor->bL[21] = luxor->bL[20]; luxor->bL[20] = luxor->bL[19]; luxor->bL[19] = luxor->bL[18]; luxor->bL[18] = luxor->bL[17]; luxor->bL[17] = luxor->bL[16]; luxor->bL[16] = luxor->bL[15];
			luxor->bL[15] = luxor->bL[14]; luxor->bL[14] = luxor->bL[13]; luxor->bL[13] = luxor->bL[12]; luxor->bL[12] = luxor->bL[11]; luxor->bL[11] = luxor->bL[10]; luxor->bL[10] = luxor->bL[9]; luxor->bL[9] = luxor->bL[8]; luxor->bL[8] = luxor->bL[7];
			luxor->bL[7] = luxor->bL[6]; luxor->bL[6] = luxor->bL[5]; luxor->bL[5] = luxor->bL[4]; luxor->bL[4] = luxor->bL[3]; luxor->bL[3] = luxor->bL[2]; luxor->bL[2] = luxor->bL[1]; luxor->bL[1] = luxor->bL[0]; luxor->bL[0] = inputSampleL * sqdrive;
		...
		//voxbox 40 db
			...
			luxor->bR[33] = luxor->bR[32]; luxor->bR[32] = luxor->bR[31];
			luxor->bR[31] = luxor->bR[30]; luxor->bR[30] = luxor->bR[29]; luxor->bR[29] = luxor->bR[28]; luxor->bR[28] = luxor->bR[27]; luxor->bR[27] = luxor->bR[26]; luxor->bR[26] = luxor->bR[25]; luxor->bR[25] = luxor->bR[24]; luxor->bR[24] = luxor->bR[23];
			luxor->bR[23] = luxor->bR[22]; luxor->bR[22] = luxor->bR[21]; luxor->bR[21] = luxor->bR[20]; luxor->bR[20] = luxor->bR[19]; luxor->bR[19] = luxor->bR[18]; luxor->bR[18] = luxor->bR[17]; luxor->bR[17] = luxor->bR[16]; luxor->bR[16] = luxor->bR[15];
			luxor->bR[15] = luxor->bR[14]; luxor->bR[14] = luxor->bR[13]; luxor->bR[13] = luxor->bR[12]; luxor->bR[12] = luxor->bR[11]; luxor->bR[11] = luxor->bR[10]; luxor->bR[10] = luxor->bR[9]; luxor->bR[9] = luxor->bR[8]; luxor->bR[8] = luxor->bR[7];
			luxor->bR[7] = luxor->bR[6]; luxor->bR[6] = luxor->bR[5]; luxor->bR[5] = luxor->bR[4]; luxor->bR[4] = luxor->bR[3]; luxor->bR[3] = luxor->bR[2]; luxor->bR[2] = luxor->bR[1]; luxor->bR[1] = luxor->bR[0]; luxor->bR[0] = inputSampleR * sqdrive;
		...
		luxor->fpdL ^= luxor->fpdL << 13; luxor->fpdL ^= luxor->fpdL >> 17; luxor->fpdL ^= luxor->fpdL << 5;
		inputSampleL += (((double)luxor->fpdL-(uint32_t)0x7fffffff) * 5.5e-36l * pow(2,expon+62));
		frexpf((float)inputSampleR, &expon);
		luxor->fpdR ^= luxor->fpdR << 13; luxor->fpdR ^= luxor->fpdR >> 17; luxor->fpdR ^= luxor->fpdR << 5;
